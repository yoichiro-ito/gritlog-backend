...
CREATE TABLE goal (
    ...
    created_at    DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at    DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    CONSTRAINT fk_goals_user FOREIGN KEY (account_id) REFERENCES account(id)
) ENGINE=InnoDB;
...
CREATE TABLE comment (
    ...
    CONSTRAINT fk_comment_author FOREIGN KEY (author_user_id) REFERENCES account(id),
    CONSTRAINT fk_comment_parent FOREIGN KEY (parent_comment_id) REFERENCES comment(id),
    KEY idx_comments_reflection (reflection_id)
) ENGINE=InnoDB;

-- reactions（コメントや振り返りへの絵文字リアクション）
--CREATE TABLE reactions (
--  id    BIGINT PRIMARY KEY AUTO_INCREMENT,
--  target_type  VARCHAR(16)  NOT NULL, -- REFLECTION or COMMENT
--  target_id  BIGINT  NOT NULL,
--  account_id    BIGINT  NOT NULL,
--  emoji   VARCHAR(64)  NOT NULL,
--  created_at DATETIME    NOT NULL DEFAULT CURRENT_TIMESTAMP,
--  CONSTRAINT fk_reactions_user FOREIGN KEY (account_id) REFERENCES users(id),
